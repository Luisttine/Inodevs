...
<head>
    ...
    <link rel="stylesheet" href="quadro.css?v=<? echo time() ?>">
</head>
<body>
    <!-- jQuery -->
...
<h1>Quadro de Presença</h1>
    ...
    <?php 
        ...
        // Do-while para contar na variável $k quantos colaboradores existem no total
        $k=0;
        do {
            $k++;
        } while($linha_colaborador=$sql_query_colaborador->fetch_assoc());

        // É adicionado novamente os dados a variável, já que foram usados para o contador acima
        $sql_query_colaborador = $conn->query($sql_code_colaborador) or die($mysqli->error);
        $linha_colaborador = $sql_query_colaborador->fetch_assoc();
...
?>
    ...
    <?php
        ...
        // É adicionado 1 aos contadores $k2 (para imprimir  todos os colaboradores daquele posto) e $colaborador (para criar uma class específica para os colaboradores de certo posto de trabalho)
        $k2++;
        $colaborador++;
